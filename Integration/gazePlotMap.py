...
def getGazeTime(FrameName, BB_list, gaze_points):
    new_BB_list = [[] for i in range(len(BB_list))]
    for P in gaze_points:
        for BB in BB_list:
            if PointTest(BB[0], BB[1], BB[0] + BB[2], BB[1] + BB[3], P[0], P[1]):
                new_BB_list[BB_list.index(BB)].append(P)  # BB_list.index(BB)

    # BBfile = open("./File_out/BB_with_time.txt", "w")
    bbox_file = open("./File_out/BB_with_time.txt", "a")
    bbox_file.write(FrameName + " | ")
    # print(BB_list, len(new_BB_list))
    bb_with_time = []
    for i in range(len(new_BB_list)):
        if len(new_BB_list[i]) != 0:
            timeCol = new_BB_list[i]  # [:, 2]
            arr = np.array(timeCol)
            timeCol = arr[:, 2]
            # timeColFormat = datetime.datetime.strptime(timeCol, '%H:%M:%S.%f')
            time_list = [datetime.datetime.strptime(t, '%H:%M:%S').time() for t in timeCol]
            min_time = min(time_list)
            max_time = max(time_list)
            time_diff = datetime.datetime.combine(datetime.date.today(), max_time) - datetime.datetime.combine(datetime.date.today(), min_time)
            time_diff = int(time_diff.total_seconds())
            bb_with_time.append([BB_list[i], time_diff])
            bbox_file.write(str(BB_list[i][0])+" "+str(BB_list[i][1])+" "+str(BB_list[i][2])+" "+str(BB_list[i][3])+" "+str(time_diff)+" | ")
...
def GazeFrameMap(coordinatesFileName):
    ...
    for frameName in imageNames:
        ...
        BB_list = bounding_box(HeatMap_OutFile, frameName, display_width, display_height)
        getGazeTime(frameName, BB_list, gazePoints)
...
def gPMain():
    ...
    if os.path.exists("./File_out/BB_with_time.txt"):
        os.remove("./File_out/BB_with_time.txt")
