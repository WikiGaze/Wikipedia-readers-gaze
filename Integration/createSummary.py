...
class Screen(object):
    ...
    def get_Coordinates(self, frameName):
        ...
        bboxFile = open("./File_out/BB_with_time.txt", "r")
        for line in bboxFile:
            ...
            if line_parts[0].__contains__(frameName):
                for i in range(1, len(line_parts) - 1):
                    ...
                    coords = [coords[0], coords[1], coords[0] + coords[2], coords[1] + coords[3], coords[4]]
    ...
    def text_Extraction(self, window_name, boxes):
        ...
        for i in range(len(boxes)):
            ...
            extracted_image = window.crop(read_box[:-1])
            ...
            if read_box[2] - read_box[0] >= 2500:
                ...
                # print('\n' + extracted_text + '\n \n' + str(read_box))
                # self.extracted_data.append("@#%"+str(read_box[len(read_box)])+"%#@")
                self.extracted_data.append([read_box[len(read_box) - 1], extracted_text])
                # sc.extracted_data = '\n'.join(sc.extracted_data)  # data.append(extracted_text)
                # = (window_name, read_box, self.pixel_info)
            elif extracted_text:
                # self.extracted_data.append("@#%"+str(read_box[len(read_box)-1])+"%#@")
                # self.extracted_data.append(extracted_text)
                self.extracted_data.append([read_box[len(read_box) - 1], extracted_text])
...
def crMain():
    ...
    # Remove redundancy
    unclean_summary = sc.extracted_data
    ...
    matches = []
    for block in unclean_summary:
        clean = sc.clean_Extracted_Text(block[1])
        clean = clean.replace("\n", ". ")
        block_sentences = clean.split(". ")
        sentence_time = block[0] / len(block_sentences)
        sentence_time = round(sentence_time, 2)
        for line in block_sentences:
            match = get_close_matches(line, original_list, cutoff=0.4)
            if len(match) != 0:
                if len(str(match[0]).split()) > 5:
                    str1_words = set(line.split())
                    str2_words = set(match[0].split())
                    common = str1_words & str2_words
                    if len(common) > len(str1_words)/2:
                        matches.append([sentence_time, match[0], 0])
            else:
                if len(line) > 30:
                    original_cut = [x[0:30] for x in original_list]
                    match = get_close_matches(line[0:30], original_cut, cutoff=0.8)
                    if len(match) != 0:
                        if len(str(match[0]).split()) > 4:
                            index = original_cut.index(match[0])
                            match = original_list[index]
                            matches.append([sentence_time, match, 0])
        ...
        if matches[i][2] == 0:
            for j in range(i+1, len(matches)):
                if matches[i][1] == matches[j][1]:
                    matches[i][0] = round(matches[i][0] + matches[j][0], 2)
                    matches[j][2] = 1
            matches_without_dup.append(matches[i])
    matches.clear()

    # Sort the original text sentences according to index and create summary
    gaze = []
    summaryFile = open("./File_out/summary.txt", "w")
    for each in matches_without_dup:
        summaryFile.write("@" + str(each[0]) + "@ " + each[1].strip() + " ")
        gaze.append(float(each[0]))

    if len(gaze) > 0:
        avg_gaze = sum(gaze)/len(gaze)
    else:
        avg_gaze = 0

    print("Average time To each each line: {:0.2f}".format(avg_gaze))
    print("Total time for text read: {:0.2f}".format(sum(gaze)))

    original_list.clear()
    matches_without_dup.clear()
    summaryFile.close()
